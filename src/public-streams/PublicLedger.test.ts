...
test('PublicLedger', async () => {
  ...
  const { result, waitForValueToChange } = renderPublicLedgerHook(() => {
...
test('useQueryAsPublic', () => {
  fetchMock.mockResponse(JSON.stringify({ access_token: PUBLIC_TOKEN }));
  const { result } = renderPublicLedgerHook(() => {
    return useQueryAsPublic(Foo);
  });
  expect(fetchMock.mock.calls.length).toEqual(1);
  expect(result.current).toEqual(undefined);
